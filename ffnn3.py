...
w1 = np.random.randn(5000, d) * np.sqrt(2. / d)
b1 = np.zeros((5000, 1))
w2 = np.random.randn(1, 5000) * np.sqrt(2. / 5000)
b2 = np.zeros((1, 1))
...
for t in range(20):  
    ...
    for i in range(num_batches):
        ...
        loss = np.maximum(0, 1 - label_batch * a2)

        dl_da2 = np.where(1 - label_batch * a2 <= 0, 0, -label_batch)  
